...
uses
  ...
  Windows, Graphics, ImgList, CommCtrl, StdCtrls;//ComCtrls;

type
  TFPicLabel = class(TCustomLabel)
  private
...
//    FImageChangeLink: TChangeLink;
    ...
    FMargLeft,FMargTop,FMargTextTop:Integer;
...
//    property Transparent: Boolean read GetTransparent write SetTransparent default True;
    ...
    property MargTextTop: Integer read FMargTextTop write FMargTextTop default 1;
//    property Color;
...
procedure TFPicLabel.SetImageList(Value: HImageList);
begin
//  if HandleAllocated then Perform(TB_SETIMAGELIST, 0, Value);
  Invalidate;
end;

procedure TFPicLabel.SetImages(Value: TCustomImageList);
begin
...
procedure TFPicLabel.SetImageIndex(Value: TImageIndex);
begin
...
procedure TFPicLabel.SetHotImageIndex(Value: TImageIndex);
begin
...
function TFPicLabel.GetTransparent: Boolean;
begin
  Result := not (csOpaque in ControlStyle);
end;

procedure TFPicLabel.SetTransparent(Value: Boolean);
begin
...
{procedure TFPicLabel.CreateParams(var Params: TCreateParams);
const
...
procedure TFPicLabel.CMBorderChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TFPicLabel.CMTextChanged(var Message: TMessage);
begin
  Invalidate;
end;

{procedure TFPicLabel.CMCtl3DChanged(var Message: TMessage);
begin
  if NewStyleControls and (FBorderStyle = bsSingle) then RecreateWnd;
  inherited;
end;

procedure TFPicLabel.CMIsToolControl(var Message: TMessage);
begin
  if not FLocked then Message.Result := 1;
end;  }

procedure TFPicLabel.WMWindowPosChanged(var Message: TWMWindowPosChanged);
//var
//  BevelPixels: Integer;
//  Rect: TRect;
begin
...
procedure TFPicLabel.SetAlignment(Value: TAlignment);
begin
  FAlignment := Value;
  Invalidate;
end;

procedure TFPicLabel.SetBevelInner(Value: TPanelBevel);
begin
  FBevelInner := Value;
//  Realign;
  Invalidate;
...
procedure TFPicLabel.SetBevelOuter(Value: TPanelBevel);
begin
  FBevelOuter := Value;
//  Realign;
  Invalidate;
...
procedure TFPicLabel.SetBevelWidth(Value: TBevelWidth);
begin
  FBevelWidth := Value;
//  Realign;
  Invalidate;
...
procedure TFPicLabel.SetBorderWidth(Value: TBorderWidth);
begin
  FBorderWidth := Value;
//  Realign;
  Invalidate;
...
procedure TFPicLabel.SetBorderStyle(Value: TBorderStyle);
begin
...
function TFPicLabel.GetControlsAlignment: TAlignment;
begin
  Result := FAlignment;
end;

{procedure TFPicLabel.AdjustClientRect(var Rect: TRect);
var
  BevelSize: Integer;
begin
...
procedure TFPicLabel.CMDockClient(var Message: TCMDockClient);
var
  R: TRect;
  Dim: Integer;
begin
...
//!!!procedure TFPicLabel.SetParentBackground(Value: Boolean);
//begin
...
function TFPicLabel.CanAutoSize(var NewWidth, NewHeight: Integer): Boolean;
begin
...
procedure Register;
begin
  RegisterComponents('Samples', [TFPicLabel]);
end;
