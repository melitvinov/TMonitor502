...
//    csSetCaption, csDoubleClicks, csReplicatable]; //csOpaque,
  { When themes are on in an application default to making
    TFLabel's paint with their ParentBackground }
...
procedure TFLabel.SetImageList(Value: HImageList);
begin
//  if HandleAllocated then Perform(TB_SETIMAGELIST, 0, Value);
  Invalidate;
end;

procedure TFLabel.SetImages(Value: TCustomImageList);
begin
...
procedure TFLabel.SetImageIndex(Value: TImageIndex);
begin
...
procedure TFLabel.SetHotImageIndex(Value: TImageIndex);
begin
...
function TFLabel.GetTransparent: Boolean;
begin
  Result := not (csOpaque in ControlStyle);
end;

procedure TFLabel.SetTransparent(Value: Boolean);
begin
...
{procedure TFLabel.CreateParams(var Params: TCreateParams);
const
...
procedure TFLabel.CMBorderChanged(var Message: TMessage);
begin
  inherited;
  Invalidate;
end;

procedure TFLabel.CMTextChanged(var Message: TMessage);
begin
  Invalidate;
end;

{procedure TFLabel.CMCtl3DChanged(var Message: TMessage);
begin
  if NewStyleControls and (FBorderStyle = bsSingle) then RecreateWnd;
  inherited;
end;

procedure TFLabel.CMIsToolControl(var Message: TMessage);
begin
  if not FLocked then Message.Result := 1;
end;  }

procedure TFLabel.WMWindowPosChanged(var Message: TWMWindowPosChanged);
//var
//  BevelPixels: Integer;
//  Rect: TRect;
begin
...
procedure TFLabel.SetAlignment(Value: TAlignment);
begin
  FAlignment := Value;
  Invalidate;
end;

procedure TFLabel.SetBevelInner(Value: TPanelBevel);
begin
  FBevelInner := Value;
//  Realign;
  Invalidate;
...
procedure TFLabel.SetBevelOuter(Value: TPanelBevel);
begin
  FBevelOuter := Value;
//  Realign;
  Invalidate;
...
procedure TFLabel.SetBevelWidth(Value: TBevelWidth);
begin
  FBevelWidth := Value;
//  Realign;
  Invalidate;
...
procedure TFLabel.SetBorderWidth(Value: TBorderWidth);
begin
  FBorderWidth := Value;
//  Realign;
  Invalidate;
...
procedure TFLabel.SetBorderStyle(Value: TBorderStyle);
begin
...
function TFLabel.GetControlsAlignment: TAlignment;
begin
  Result := FAlignment;
end;

{procedure TFLabel.AdjustClientRect(var Rect: TRect);
var
  BevelSize: Integer;
begin
...
procedure TFLabel.CMDockClient(var Message: TCMDockClient);
var
  R: TRect;
  Dim: Integer;
begin
...
//!!!procedure TFLabel.SetParentBackground(Value: Boolean);
//begin
...
function TFLabel.CanAutoSize(var NewWidth, NewHeight: Integer): Boolean;
begin
...
procedure Register;
begin
  RegisterComponents('Samples', [TFLabel]);
end;
