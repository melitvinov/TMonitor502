...
uses
  ...
  GIFDef, GIFComponent, FPicLabel, FPicPanel,TSGrid, Grids_ts;   //,Grids_ts
//---------------------------------------------------
//       Определения входов
...
with ParentCtr as TFClimat510 do
 ...
 TipSensor,TipGoTask,SensorRCS:
  ...
  prognT:tInInt:=GetProgn(pHot^[DZ_itTAir+tIndex]);
  AbsRH: tInInt:=GetAbsH(pHot^[DZ_itTAir+tIndex],pHot^[DZ_itRH+tIndex]);
  DDWP: tInInt:=GetDDWP(pHot^[DZ_itTAir+tIndex],pHot^[DZ_itRH+tIndex]);
  DDWPS: tInInt:=GetDDWP(pHot^[DZ_itTSheet+tIndex],pHot^[DZ_itRH+tIndex]);
  TR: tInInt:=GetTR(pHot^[DZ_itTAir+tIndex],pHot^[DZ_itRH+tIndex]);
...
//================================================================================
var  XNamesCalibr120:array [1..10] of TXNames=(   //XCalibr120Count
//1
(Name:'Тип';Frm:ComboSS;Ed:'';Index:0;Cfg:0;Kind:0;Min:560;Max:572), //xcb
(Name:'Вход';Frm:SS;Ed:' ';Index:1;Cfg:0;Kind:0;Min:0;Max:32), //xcb
(Name:'Выход';Frm:SS;Ed:' ';Index:2;Cfg:0;Kind:0;Min:0;Max:15), //xcb
(Name:'Коррек';Frm:SS;Ed:' ';Index:3;Cfg:0;Kind:0;Min:0;Max:7), //xcb
(Name:'Напряжение 1';Frm:SSSS;Ed:'mV';Index:4;Cfg:0;Kind:0;Min:-0.01;Max:10000), //xcb
(Name:'Эталон 1';Frm:SS;Ed:'';Index:6;Cfg:0;Kind:0;Min:0;Max:0), //xcb
(Name:'Напряжение 2';Frm:SSSS;Ed:'mV';Index:8;Cfg:0;Kind:0;Min:-0.01;Max:10000), //xcb
(Name:'Эталон 2';Frm:SS;Ed:'';Index:10;Cfg:0;Kind:0;Min:0;Max:0), //xcb
(Name:'Принято';Frm:SS;Ed:'';Index:0;Cfg:0;Kind:0;Min:-0.01;Max:10000), //xcb
(Name:'Вычислено';Frm:SS;Ed:'';Index:0;Cfg:0;Kind:0;Min:0;Max:0) //xcb
  );
//NameConfiguration
var  ZNamesCalibr120:array [1..3] of TXNames=(
(Name:'Рассадная';Frm:SS;Ed:'';Index:2;Cfg:0;Kind:0;Min:0;Max:15), //xcb
(Name:'Зона 3';Frm:SS;Ed:'';Index:4;Cfg:0;Kind:0;Min:0;Max:15), //xcb
(Name:'Салатная';Frm:SS;Ed:'';Index:6;Cfg:0;Kind:0;Min:0;Max:15) //xcb
 );
const MaxCountY=DZ_SumConfig;
var  YNamesCalibr120:array [1..MaxCountY] of TNameConst;

procedure TCalInSens_DZ.SetChan;
var y,yC,tg:integer;
begin
  yC:=0; tg:=0;
  for y:=1 to DZ_SumConfig do //ParentCtr.Block[0].CountY do
     begin
     if NameConfiguration[y].TipSens=TipCalibr then // ParentCtr.Block[0].ConstNames[y].TipSens=TipCalibr then
          begin
          yC:=yC+1;
          YNamesCalibr120[yC]:=NameConfiguration[y];  //ParentCtr.Block[0].ConstNames[y];
          YNamesCalibr120[yC].Index:=(yC-1)*12;//c1CalibrSize;
          YNamesCalibr120[yC].Tag:=y;
//          ParentCtr.Block[0].ConstNames[y].Tag:=yC*2;
          if yC >= MaxCountY then break;
          end;
     end;
     CountY:=yC;
end;
...
begin
  CountX := 10;//ParentCtr.SumZone;
  CountY := DZ_SumVisibleInSens*DZ_SumParsSens;
  SetChan;
  SizeCol := 12*DZ_SumInSens;//sizeFullCalibr;  //eMAX_SUM_SENS * sizeCalibrSensor;
  FullSize := SizeCol*DZ_MAX_SUM_ZONE;
  SendByte := FullSize;
  Pref := 'Калибровки тепл';
  NameBlock := 'Калибровка тепличных датчиков';
  ConstNames:=Addr(YNamesCalibr120[1]);//NameInSensorsC510[1]);
  XNames:=Addr(XNamesCalibr120[1]); //XNamesSensC510[1]);
  XNamesCount:=CountX;
  _ZNames:=Addr(ZNamesCalibr120[1]);
  _ZNamesCount:=3;
end;
...
begin
  ...
  FullSize := SizeCol;
  SendByte := FullSize;
  Pref := 'Калибровки метео';
  NameBlock := 'Калибровка метео датчиков';
...
begin
     ...
     SizeCol := DZ_sizeTepl;
     FullSize := DZ_iTepl+SizeCol*DZ_MAX_SUM_ZONE;
     SendByte := 1;
     Pref := 'Аварии';
     NameBlock := 'Аварии';
